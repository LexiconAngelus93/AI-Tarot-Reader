...
import com.tarotreader.domain.usecase.GenerateReadingUseCase
...
class GenerateReadingUseCaseTest {
    
    @Test
    fun testGenerateReading() = runBlocking {
        // Create mock repository
        val repository = object : TarotRepository {
            override suspend fun getAllCards(): List<TarotCard> = emptyList()
            
            override suspend fun getCardsByDeck(deckId: String): List<TarotCard> {
                return listOf(
                    TarotCard(
                        id = "0_rider_waite",
                        name = "The Fool",
        ...
        // Create the use case
        val useCase = GenerateReadingUseCase(repository)
        ...
        // Generate a reading
        val reading = useCase("rider_waite", "three_card", cardDrawings)
        ...
        assertEquals(3, reading.cardDrawings.size)
